...
void InstructionsMenuState::ListenToEvent(Event * _event)
{
...
void InstructionsMenuState::HandleInput(Input_KeyEvent* _keyEvent)
{
...
void InstructionsMenuState::Render()
{
...
void InstructionsMenuState::Enter()
{
	Subscriber* sub = this;
	...
	m_menuHandler.LoadInstructionsMenu(m_gui, m_fnt->GetResource(), m_sndMenu->GetResource(), m_sprCursor->GetResource());
}

void InstructionsMenuState::Exit()
{
	Subscriber* sub = this;
	InputManager::GetInstance()->RemoveSubscriber(sub);
	m_menuHandler.DeleteInstructionsMenu(m_gui);

	GraphicsManager::GetInstance()->ResetGUI();
	m_gui = nullptr;
}

void InstructionsMenuState::Update(float dt)
{
	int id;
	static int lastid = 0;
...
void InstructionsMenuState::UpdateBG(float dt)
{
	float tx, ty;
	static float t = 0.0f;
...
void InstructionsMenuState::InitializeBG()
{
